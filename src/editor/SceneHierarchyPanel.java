...
public class SceneHierarchyPanel {
    ...
    private void drawComponents(Entity entity){
        ...
        drawComponent("Material", entity, MaterialComponent.class, (component) -> {
            String[] filter = new String[] { "png", "jpg", "jpeg", "tga", "png", "jpg", "jpeg", "tga" };
            int id = 0;
            ImGui.text("Diffuse Map");
            ImGui.pushID(id++);
            if(ImGui.button("Browse")){
                JFileChooser jfc = new JFileChooser(FileSystemView.getFileSystemView().getHomeDirectory());
                jfc.setFileFilter(new FileNameExtensionFilter("Image Files", filter));
                int result = jfc.showOpenDialog(null);
                if(result == JFileChooser.APPROVE_OPTION){
                    File selectedFile = jfc.getSelectedFile();
                    String filepath = selectedFile.getPath();
                    // todo destroy current texture ?
                    component.diffuseMap = AssetManager.loadAndAddTexture2D(filepath, filepath, true,
            ...
            if(ImGui.button("Browse")){
                ...
                if(result == JFileChooser.APPROVE_OPTION){
                    ...
                    String filepath = selectedFile.getPath();
                    // todo destroy current texture ?
                    component.specularMap = AssetManager.loadAndAddTexture2D(filepath, filepath, true,
                            GL33.GL_LINEAR_MIPMAP_LINEAR, GL33.GL_LINEAR, GL33.GL_REPEAT, GL33.GL_REPEAT);
                }
            }
            ImGui.popID();
            ...
            if(ImGui.button("Browse")){
                ...
                if(result == JFileChooser.APPROVE_OPTION){
                    ...
                    String filepath = selectedFile.getPath();
                    // todo destroy current texture ?
                    component.normalMap = AssetManager.loadAndAddTexture2D(filepath, filepath, true,
                            GL33.GL_LINEAR_MIPMAP_LINEAR, GL33.GL_LINEAR, GL33.GL_REPEAT, GL33.GL_REPEAT);
                }
            }
            ImGui.popID();
