...
class Listener implements EntityListener{
    ...
    public void entityAdded(Entity entity) {
        Entity parent = entity.getComponent(RelationshipComponent.class).parent;
        String pName = "None";
        if(parent != null)
            pName = parent.getComponent(TagComponent.class).tag;
        System.out.println(entity.getComponent(TagComponent.class).tag + " child of " + pName + " added");
    }
    ...
    public void entityRemoved(Entity entity) {
        ...
        if(parent != null)
            pName = parent.getComponent(TagComponent.class).tag;
        System.out.println(entity.getComponent(TagComponent.class).tag + " child of " + pName + " removed");

        var mesh = entity.getComponent(MeshComponent.class);
        if(mesh != null){
            for(var vbo : mesh.vao.getVertexBuffers()){
                vbo.destroy();
            }
            mesh.vao.getVertexBuffers().clear();
            mesh.vao.getIndexBuffer().destroy();
            mesh.vao.destroy();
        }
    }
}
